...
namespace NCL {
	...
	namespace CSC8503 {
		...
		protected:
			...
			// split node into 4 child nodes that fill the same area of original node
			void Split() {
				Vector2 halfSize = size / 2.0f;		// * 0.5f??
			...
			void OperateOnContents(QuadTreeFunc& func) {
				// check node has children and recursively call until we reach a leaf node i.e. no children
				if (children)
					for (int i = 0; i < 4; ++i)
