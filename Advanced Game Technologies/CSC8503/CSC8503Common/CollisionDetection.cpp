...
bool CollisionDetection::RayIntersection(const Ray& r, GameObject& object, RayCollision& collision) {
	// every game object should have a transform so reference
	const Transform& transform = object.GetConstTransform();
	// may not have a collision volume so use pointer and check
...
bool CollisionDetection::ObjectIntersection(GameObject* a, GameObject* b, CollisionInfo& collisionInfo) {
	...
	// @TODO just for now so OBBs at least collide with ABBs until OBBABB is implemented
...
bool CollisionDetection::AABBTest(const Vector3& posA, const Vector3& posB, const Vector3& halfSizeA, const Vector3& halfSizeB) {
	// distance between AABBs on each axis
	Vector3 delta = posB - posA;
	// sum of box sizes
	Vector3 totalSize = halfSizeA + halfSizeB;

	// if distances on all axis between AABBs < sum of box sizes
	// abs always returns positive value, we don't care if box A is left of box B etc. e.g. abs(-10) = 10
